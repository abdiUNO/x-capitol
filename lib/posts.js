...
export function fetchPostContent() {
  ...
  const allPostsData = fileNames
    ...
    .map((fileName) => {
      ...
      console.log(`matterResult: ${JSON.stringify(matterResult)}`);

      const matterData = {
        content: matterResult.content,
        timeFormatted: moment(matterResult.data.date).fromNow(),
        ...matterResult.data,
      };
...
export function countPosts(tag) {
  return fetchPostContent().filter((it) => !tag || (it.tags && it.tags.includes(tag)))
    .length;
}
